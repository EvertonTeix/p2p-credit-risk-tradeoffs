...
# =============================================================================
# 1. FUNÇÕES AUXILIARES (MÉTRICAS TRADICIONAIS E DE FAIRNESS)
# =============================================================================
...
def calculate_fairness_metrics(y_true, y_pred, protected_attr, priv_value):
    y_true = np.array(y_true)
    y_pred = np.array(y_pred)
    protected_attr = np.array(protected_attr)
    
    priv_idx = (protected_attr == priv_value)
    unpriv_idx = (protected_attr != priv_value)
    
    tn_p, fp_p, fn_p, tp_p = confusion_matrix(y_true[priv_idx], y_pred[priv_idx], labels=[0, 1]).ravel()
    tn_u, fp_u, fn_u, tp_u = confusion_matrix(y_true[unpriv_idx], y_pred[unpriv_idx], labels=[0, 1]).ravel()
    
    sr_p = (tp_p + fp_p) / (tp_p + fp_p + tn_p + fn_p) if (tp_p + fp_p + tn_p + fn_p) > 0 else 0
    sr_u = (tp_u + fp_u) / (tp_u + fp_u + tn_u + fn_u) if (tp_u + fp_u + tn_u + fn_u) > 0 else 0
    ...
    fnr_p = fn_p / (tp_p + fn_p) if (tp_p + fn_p) > 0 else 0
    fnr_u = fn_u / (tp_u + fn_u) if (tp_u + fn_u) > 0 else 0
    
    er_p = (fp_p + fn_p) / (tp_p + fp_p + tn_p + fn_p) if (tp_p + fp_p + tn_p + fn_p) > 0 else 0
    er_u = (fp_u + fn_u) / (tp_u + fp_u + tn_u + fn_u) if (tp_u + fp_u + tn_u + fn_u) > 0 else 0

    return {
        'Disparate Impact Ratio': sr_u / sr_p if sr_p > 0 else np.nan, 
        'Statistical Parity Diff': sr_u - sr_p,
        'Equal Opp. Diff': tpr_u - tpr_p,
        'Error Rate Gap': er_u - er_p, 
        'FPR Gap': fpr_u - fpr_p,
        'FNR Gap': fnr_u - fnr_p
    }
...
# =============================================================================
# 2. CARREGAMENTO E PRÉ-PROCESSAMENTO (LIMPEZA INICIAL)
# =============================================================================
...
# =============================================================================
# 3. CONFIGURAÇÕES DAS FEATURES PROTEGIDAS E GRUPOS
# =============================================================================

config_protegida = {
    'Gender': {
        'binary_col': 'GenderBinary',
        'lambda_func': lambda x: 1 if x == 2 else 0, 
        'priv_value': 0 
    },
    'Age': {
        'binary_col': 'AgeBinary',
        'lambda_func': lambda x: 1 if x < 40 else 0,
        'priv_value': 0
    },
    'MaritalStatus': {
        'binary_col': 'MaritalStatusBinary',
        'lambda_func': lambda x: 1 if x == 2 else 0,
        'priv_value': 0
    },
    'Education': {
        'binary_col': 'EducationBinary',
        'lambda_func': lambda x: 1 if x in [2, 4, 5] else 0,
        'priv_value': 0
    }
}

resultados_fairness_globais = []
resultados_performance_globais = []
base_output_dir = '../outputs/DIR_Outputs'
...
# =============================================================================
# 4. LOOP PRINCIPAL: DIR, TREINAMENTO, XAI E FAIRNESS/PERFORMANCE
# =============================================================================

cenarios = list(config_protegida.keys()) + ['All']
...
for cenario in cenarios:
    ...
    df_dir = database.copy()
    
    if cenario != 'All':
        feature_name = cenario
        config = config_protegida[cenario]
        bin_col = config['binary_col']
        df_dir[bin_col] = df_dir[feature_name].apply(config['lambda_func'])
        
        dataset_aif = BinaryLabelDataset(
            df=df_dir, label_names=['Status'], protected_attribute_names=[bin_col]
        )
        
        print(f"-> Aplicando Disparate Impact Remover em {feature_name}...")
        dir_transformer = DisparateImpactRemover(repair_level=1.0)
        df_dir = dir_transformer.fit_transform(dataset_aif).convert_to_dataframe()[0]
        df_dir = df_dir.drop(columns=[feature_name, bin_col], errors='ignore')
        
        X_original_feature = database[feature_name] 
    else:
        print("-> Aplicando Disparate Impact Remover em TODAS as features simultaneamente...")
        for f_name, f_config in config_protegida.items():
            bin_col = f_config['binary_col']
            df_dir[bin_col] = df_dir[f_name].apply(f_config['lambda_func'])
            dataset_aif = BinaryLabelDataset(
                df=df_dir, label_names=['Status'], protected_attribute_names=[bin_col]
            )
            dir_transformer = DisparateImpactRemover(repair_level=1.0)
            df_dir = dir_transformer.fit_transform(dataset_aif).convert_to_dataframe()[0]
            df_dir = df_dir.drop(columns=[f_name, bin_col], errors='ignore')
    ...
    cols_cat_existentes = [c for c in cols_cat if c in df_dir.columns]
    
    df_dir = pd.get_dummies(df_dir, columns=cols_cat_existentes, prefix=cols_cat_existentes, drop_first=True)
    for col in df_dir.columns:
        if df_dir[col].dtype == 'bool':
            df_dir[col] = df_dir[col].astype(int)
            
    corr_matrix = df_dir.corr().abs()
    upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))
    colunas_remover = [col for col in upper.columns if any(upper[col] >= 0.7)]
    df_dir.drop(columns=colunas_remover, inplace=True, errors='ignore')
    
    y = df_dir['Status']
    X = df_dir.drop('Status', axis=1)
    features_names_final = X.columns
    
    print("-> Iniciando KFold e GridSearch...")
    ...
    y_test_final = None
    X_test_unscaled_feature = None
    
    if cenario == 'All':
        metrics_per_fold = {
            'Decision Tree': {'fairness': {f: [] for f in config_protegida.keys()}, 'performance': []},
            'Random Forest': {'fairness': {f: [] for f in config_protegida.keys()}, 'performance': []},
            'Logistic Regression': {'fairness': {f: [] for f in config_protegida.keys()}, 'performance': []},
            'XGBoost': {'fairness': {f: [] for f in config_protegida.keys()}, 'performance': []}
        }
    else:
        metrics_per_fold = {
            'Decision Tree': {'fairness': [], 'performance': []},
            'Random Forest': {'fairness': [], 'performance': []},
            'Logistic Regression': {'fairness': [], 'performance': []},
            'XGBoost': {'fairness': [], 'performance': []}
        }
    ...
    for train_index, test_index in kfold.split(X):
        ...
        # --- CALCULANDO MÉTRICAS DO FOLD ATUAL ---
        modelos_fold = {
        ...
        for m_name, (preds, probas) in modelos_fold.items():
            pm = analyse_metrics(preds, probas, y_test)
            metrics_per_fold[m_name]['performance'].append(pm)
            
            if cenario == 'All':
                for f_name, f_config in config_protegida.items():
                    X_test_unscaled_f = database[f_name].iloc[test_index].apply(f_config['lambda_func']).copy()
                    fm = calculate_fairness_metrics(y_test, preds, X_test_unscaled_f, f_config['priv_value'])
                    metrics_per_fold[m_name]['fairness'][f_name].append(fm)
            else:
                X_test_unscaled_feature = X_original_feature.iloc[test_index].apply(config['lambda_func']).copy()
                fm = calculate_fairness_metrics(y_test, preds, X_test_unscaled_feature, config['priv_value'])
                metrics_per_fold[m_name]['fairness'].append(fm)
    ...
    # -------------------------------------------------------------------------
    # FAIRNESS METRICS E PERFORMANCE METRICS (MÉDIAS)
    # -------------------------------------------------------------------------
    print(f"-> Calculando Médias de Fairness e Performance para {cenario}...")
    
    for m_name in metrics_per_fold.keys():
        # Calcula e salva Performance (Média dos 5 folds)
        df_pm = pd.DataFrame(metrics_per_fold[m_name]['performance'])
        avg_pm = df_pm.mean(skipna=True).to_dict()
        avg_pm['Feature'] = cenario
        avg_pm['Model'] = m_name
        resultados_performance_globais.append(avg_pm)

        # Calcula e salva Fairness (Média dos 5 folds)
        if cenario == 'All':
            for f_name in config_protegida.keys():
                df_fm = pd.DataFrame(metrics_per_fold[m_name]['fairness'][f_name])
                avg_fm = df_fm.mean(skipna=True).to_dict()
                avg_fm['Feature'] = f"All ({f_name})"
                avg_fm['Model'] = m_name
                resultados_fairness_globais.append(avg_fm)
        else:
            df_fm = pd.DataFrame(metrics_per_fold[m_name]['fairness'])
            avg_fm = df_fm.mean(skipna=True).to_dict()
            avg_fm['Feature'] = cenario
            avg_fm['Model'] = m_name
            resultados_fairness_globais.append(avg_fm)

    # -------------------------------------------------------------------------
    # GERAÇÃO XAI (SHAP E LIME)
    # -------------------------------------------------------------------------
    print(f"-> Gerando explicações XAI (SHAP e LIME) para {cenario}...")
    
    X_test_df = pd.DataFrame(X_test_final, columns=features_names_final)
    
    # Amostragem para o SHAP não exceder a memória
    ...
    for sigla, modelo in modelos_obj.items():
        dir_shap = f'{base_output_dir}/Feature_{cenario}/plots_SHAP/{sigla}'
        dir_lime = f'{base_output_dir}/Feature_{cenario}/plots_LIME'
...
# =============================================================================
# 5. RESULTADOS GLOBAIS DE FAIRNESS E PERFORMANCE (LATEX E DATAFRAME)
# =============================================================================

print("\n" + "="*80)
print("TODAS AS FEATURES PROCESSADAS COM SUCESSO!")
print("="*80)

from IPython.display import display

# --- TABELA DE FAIRNESS ---
df_fairness = pd.DataFrame(resultados_fairness_globais)
cols_fair = ['Feature', 'Model', 'Disparate Impact Ratio', 'Statistical Parity Diff', 'Equal Opp. Diff', 'Error Rate Gap']
df_fairness_revisor = df_fairness[cols_fair]

# Substituindo NaN por N/A para manter rigor metodológico na divisão por zero
df_fairness_revisor['Disparate Impact Ratio'] = df_fairness_revisor['Disparate Impact Ratio'].astype(object).fillna('N/A')

print("\n=== TABELA DE MÉTRICAS DE JUSTIÇA (FAIRNESS) PARA O REVISOR 1 ===")
display(df_fairness_revisor)

print("\n=== CÓDIGO LATEX (FAIRNESS) ===")
print(df_fairness_revisor.to_latex(index=False, escape=False))


# --- TABELA DE PERFORMANCE ---
df_performance = pd.DataFrame(resultados_performance_globais)
cols_perf = ['Feature', 'Model', 'Accuracy', 'Precision', 'Recall', 'F1 Score', 'AUC']
df_performance = df_performance[cols_perf]

os.makedirs('../outputs/DIR_Outputs', exist_ok=True)
df_fairness.to_csv('../outputs/DIR_Outputs/dir_fairness_global.csv', index=False)
df_performance.to_csv('../outputs/DIR_Outputs/dir_performance_global.csv', index=False)

print("\n=== TABELA DE MÉTRICAS DE PERFORMANCE (TRADICIONAIS) ===")
display(df_performance.round(4))

print("\n=== CÓDIGO LATEX (PERFORMANCE) ===")
